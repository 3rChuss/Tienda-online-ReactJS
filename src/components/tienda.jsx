...
class Tienda extends React.Component{
    ...
    render(){
        ...
        if(this.state.resultadoBusqueda != 0)
        {
            console.log('mostramos solo la busqueda');
    ...
    buscarProducto(e){
        ...
        for (let index = 0; index < Productos.length; index++) {
            ...
            if(buscado.includes(e.target.value.toLowerCase())) resultado.push(element);
            else error = "No se ha encontrado productos con ese nombre :)";
        }
        this.setState({resultadoBusqueda : resultado, err : error})
